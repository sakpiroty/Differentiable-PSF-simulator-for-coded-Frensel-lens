...
class LensSystem:
    ...
    def simulatePSF(self, mask: torch.tensor, cond: str, n_rays = NRAYS):
        ...
        if os.path.exists("./log/ray_behind_Fresnel_plane.pt"):
            ray = torch.load("./log/ray_behind_Fresnel_plane.pt", map_location=device)
        ...
        for object_z in range(min_depth, max_depth + 1, 10):
            ...
            print(f"Entire transmission rate: {aft/bef}")
            zs.append(object_z)
            intensity_depth.append((aft/bef).item())
    ...
    def optim_mask(self):
        ...
        # center = MASKSIZE // 2
        # ini_mask = ini_mask * ((torch.sqrt((xx - center) ** 2 + (yy - center) ** 2) <= (MASKSIZE/16)).float().to(device))
        # save_tensorimage(ini_mask, min = 0, max = 1,  filepath = f"Mask_SA.png")
        # ini_mask = (Image.open(f"Mask_SA.png").convert("L"))
        # transform = transforms.Compose(
        #     [
        #         transforms.ToTensor(),
        #         transforms.Resize(MASKSIZE),
