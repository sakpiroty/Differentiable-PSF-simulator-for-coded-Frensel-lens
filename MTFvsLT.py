...
file_path = "./data/thru-focus.csv"
df = pd.read_csv(file_path)
targetloc = torch.tensor(df[["x", "y", "z"]].values, dtype=torch.float32, device = device)
min_depth = (torch.min(targetloc[:,2])/10 + 0.5).to(torch.long) * 10
max_depth = (torch.max(targetloc[:,2])/10 + 0.5).to(torch.long) * 10
...
def save_tensorimage_pseudo_color(tensor, min, max, filepath):
    # テンソルが2次元であることを確認
    if tensor.ndim != 2:
        raise ValueError("Dimension of the saved tensor must be 2.")
    ...
    graymap = tensor_uint8.cpu().numpy()
    coloredmap = cv2.applyColorMap(graymap, cv2.COLORMAP_TURBO)
    # CV Matに変換，疑似カラー
    cv2.imwrite(filepath, np.array(coloredmap))
...
class LensSystem:
    ...
    # ray_in: 入射レイ
    def raytrace_from_object(self, n_rays = NRAYS) -> torch.tensor:
        ...
        ray = Ray(rayin_origin, rayin_direction, rayin_roi, rayin_intensity)
    ...
    def simulatePSF(self, mask: torch.tensor, cond: str, n_rays = NRAYS):
        s_time = time.time()
        intensity_depth = []
        zs = []
        ...
        if os.path.exists("./log/ray_behind_Fresnel_plane_sparse.pt"):
            ray = torch.load("./log/ray_behind_Fresnel_plane_sparse.pt", map_location=device)

        else:
            # レイトレース計算
            ray = self.raytrace_from_object()
            print("Tracing to the bottom surface of the Fresnel Lens is successfully finished.")
            torch.save(ray, os.path.join("./log/ray_behind_Fresnel_plane_sparse.pt"))

        #ray.origin, n, t = lens.intersect_plane(ray, 10.0, 0, 250)

        if (cond == 'womask'):
            print("Mask process are not performed.")
        else:
            # 空間マスク
            ...
            # transmission = (torch.nansum(ray.intensity[:,:,0]))/(torch.nansum(tmp_intensity)) # 光線の減衰比率

        ave_depth_plane_mtf = 0
        depth_ct = 0
        ave_trans = 0
        for object_z in range(min_depth, max_depth + 1, 10):
            MAGNIFY = object_z / (-image_z)
            # 結像位置まで計算
            p, n, t = lens.intersect_plane(ray, object_z, 0, 500)
            pix_world_xys = p/pixel_size/MAGNIFY
            result_xys = p/pixel_size/MAGNIFY
            result_intensity = ray.intensity[:,:,0]
            ...
            # bef = result_intensity.size()[0] * result_intensity.size()[1] # intensities that light source originally had
            aft = torch.nansum(intensities)  # intensities that reaches the target plane
            print(f"Entire transmission rate: {aft}")
            ave_trans += aft.item()
            zs.append(object_z)
            intensity_depth.append((aft).item())
            ...
            #save_tensorimage(psftensor, min = torch.min(psftensor), max = torch.max(psftensor), filepath =  f"./psfs/"+cond+f"_{object_z}.png")
            #save_tensorimage_pseudo_color(psftensor, min = torch.min(psftensor), max = torch.max(psftensor), filepath =  f"./psfs/pseudo_c_"+cond+f"_{object_z}.png")
            ...
            ave_all_mtf = 0
            for n_psf in range(25):
                centerpsf = psfpatches[n_psf]
                # PSFをフーリエ変換してMTFを取得
                norm_psfpatch = prev_normalize_image(centerpsf)
                ...
                save_tensorimage_pseudo_color(magnitude_spectrum, min = 0, max = torch.max(magnitude_spectrum), filepath = f"./MTF/{n_psf+1}.png")
                ave_all_mtf += (magnitude_spectrum.mean()).item()
                # ave_all_mtf += (magnitude_spectrum < 1e-4).sum().item()
                
            ave_depth_plane_mtf += ave_all_mtf/25
            depth_ct += 1
            # for i, psf in enumerate(psfpatches):
                # save_tensorimage(psf, min = torch.min(psf), max = torch.max(psf),  filepath = f"./psfs/"+cond+f"_{object_z}_{i+1}.png")
                # print(torch.min(psf))
        
        ave_of_mtf.append(ave_depth_plane_mtf / depth_ct)
        transmittances.append(ave_trans / depth_ct)
        # データフレームを作成
        df = pd.DataFrame([zs, intensity_depth])
        df.to_csv("./data/"+cond+"_intensity_depth.csv")

    def optim_mask(self, m_size = None, tau = None):

        # ## マスクなしの場合
        # cond = "womask"
        # ini_mask = (Image.open(f"Mask_open.png").convert("L"))
        ...
        # # # マスクありの場合
        if (cond == "wmask"):
            # ini_mask = (Image.open(f"64_{tau}_forNow.png").convert("L"))
            ini_mask = (Image.open(f"64_0.5_forNow_copy.png").convert("L"))
            transform = transforms.Compose(
                [
                    transforms.ToTensor(),
                    transforms.Resize(MASKSIZE),
        ...
        # xx, yy = torch.meshgrid(torch.arange(MASKSIZE), torch.arange(MASKSIZE), indexing='ij')
        # center = MASKSIZE // 2
        # inner = (torch.sqrt((xx - center) ** 2 + (yy - center) ** 2) <= (MASKSIZE/8)).to(device)
        # ini_mask = torch.where(inner, 1, ini_mask)
        # save_tensorimage(ini_mask, min = 0, max = 1,  filepath = f"Mask_shiva.png")
        # ###

        self.simulatePSF(ini_mask, cond)


####
## SIMULATION OF AUTO IRIS
#### 

# cond = "sa"
# transmittances = []
# ave_of_mtf = []
# for m_size in range(MASKSIZE//2 + 1):
#     lsys = LensSystem(['data/fresnel.csv'])
#     lsys.optim_mask(m_size)

# transmittances = [(n / transmittances[-1]) for n in transmittances]

# import csv

# result_list = [transmittances, ave_of_mtf]
# with open("MTFvsLT_sa.csv", 'w', newline = '') as file:
...
for tau in tau_list:
    lsys = LensSystem(['data/fresnel.csv'])
    lsys.optim_mask(tau = tau)

cond = "sa"
lsys = LensSystem(['data/fresnel.csv'])
lsys.optim_mask(m_size = MASKSIZE//2)
